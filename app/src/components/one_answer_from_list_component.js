...
class OneAnswerFromList extends Component {
  constructor(props) {
    super(props);
    this.state = {...props};
  ...
  componentDidMount() {
    if(!Object.keys(this.state.component.data).length) {
      ...
      let answer = (Array.isArray(newState.component.data.answer)) ? newState.component.data.answer[0] : newState.component.data.answer;
      newState.component.data.answer = newState.component.data.options.some((option) => option == answer) ? answer : '';
      ...
      return (
        <div onDoubleClick={this.switchEdit}>
          ...
          {content.options.map((answer, answerId) => (
            <div key = {answerId} >
              <input type="radio"
                name={'options_' + this.state.index}
                id={'answer_' + answerId} value={answer}
                checked={content.answer ? (content.answer == answer) : false}
...
export default connect(mapStateToProps, mapDispatchToProps)(OneAnswerFromList);
